...
namespace ScriptedTextAnimator.Rendering
{
    public class GifClass
    {
        ...
        {
            LoadGifPicture((Bitmap) Image.FromFile(filename));
        }
        ...
        private void AnalyzeScreenDescriptor(List<byte> gifData)
        {
            ...
            var globalColorTableFollows = (m_ScreenDescriptor[4] & 0x80) != 0;

            if (globalColorTableFollows)
            {
                var pixel = m_ScreenDescriptor[4] & 0x07;

                var lengthOfColorTableInByte = 3*((int) Math.Pow(2, pixel + 1));

                for (var i = 0; i < lengthOfColorTableInByte; i++)
        ...
        private void AnalyzeImageDescriptor(List<byte> gifData)
        {
            ...
            var localColorMapFollows = (m_ImageDescriptor[9] & 0x80) != 0;

            if (localColorMapFollows)
            {
                var pixel = m_ImageDescriptor[9] & 0x07;
            ...
            {
                var lastThreeBitsOfGlobalTableDescription = m_ScreenDescriptor[4] & 0x07;
        ...
        private void GetImageData(List<byte> gifData)
        {
            ...
            while (gifData[0] != 0x00)
            {
                ...
                for (var i = 0; i <= countOfFollowingDataBytes; i++)
